...
class FolderDownloadOld extends AbstractDownloadOld {

	private FolderContents _contentsReceived = null;

	@SuppressWarnings("unused") private WeakContract _folderContentConsumerContract;

	FolderDownloadOld(File folder, Hash hashOfFolder, Seal source, boolean copyLocalFiles) {
	...
	private Download startSpinOffDownload(FileOrFolder entry) {
		...
		? new FolderDownloadOld(new File(_path, entry.name), entry.hashOfContents, _source, _copyLocalFiles)
		: new FileDownloadOld(new File(_path, entry.name), entry.size, entry.lastModified, entry.hashOfContents, _source, _copyLocalFiles);	
	}

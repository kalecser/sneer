...
import sneer.bricks.snapps.games.go.impl.gui.gameSetup.GoSetupInvitation;
...
class GoMainImpl implements GoMain {
	...
	private void setupContextMenu() {
		my(ContactActionManager.class).addContactAction(new ContactAction() {
			...
			public void run() {
				...
				new GoSetupInvitation(_adversary, gameId);				
			}
	...
	private void handleInviation(final GoInvitation message) {
		...
		setupPlayers(stoneColor,message._size, message._gameId);
	...
	private void setupPlayers(final ByRef<StoneColor> stoneColor, final int size, final int gameId) {
		...
		@Override public void run() {
			...
			Player remotePlayer = new RemotePlayerOnSneer(gameId,remoteColor,_moveRegister,_ackRegister);
			Player localPlayer = new GuiPlayer(gameId,stoneColor.value, size, new SneerTimerFactory()); 
